    ...
    const handleNewRequest = async (data) => {
      ...
      try {
        ...
        const areCoordsEqual = (a, b, precision = 6) => {
          ...
          return (
            round(a.lat) === round(b.lat()) && round(a.lng) === round(b.lng())
          );
        };
        ...
        if (driverCoords) {
          ...
          if (isSameLoc) {
            ...
            const { distance, eta } = await getRouteDetails(
              driverCoords,
              pickupCoords
            );
            driverToPickupDistance = distance;
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          setLocationStatus(
            "Detected your current location below. If it looks incorrect, enter your address manually."
          );

          await axios.post(
            "http://localhost:3001/api/drivers/update-location",
            {
              driverId: driver._id,
              lng: coords.lng,
              lat: coords.lat,
  ...
  const handleManualLocationSubmit = async (e) => {
    ...
    try {
      ...
      if (!results || results.length === 0) {
        alert(
          "No matching address found. Please enter a more precise location."
        );
        return;
      }
    ...
    <div className="driver-dashboard-wrapper">
        ...
        <div className="fullscreen-map">
          ...
          >
            {isLoaded && driverLatLng ? (
              ...
              console.log(
                "⛔️ Marker not rendering: driverLatLng not available."
              )
            )}
      ...
      <div className="driver-overlay-content">
        <div className="driver-profile">
          ...
          <div>
            ...
            <p className={`driver-status-badge ${driverStatus}`}>
        ...
        <div className="driver-status-toggle">
          <button
            ...
            onClick={() => changeDriverStatus("online")}
          >
            Go Online
          </button>
          <button
            className={`status-btn ${
              driverStatus === "offline" ? "active" : ""
            }`}
            onClick={() => changeDriverStatus("offline")}
          >
            Go Offline
          </button>
        </div>
              ...
              {({ getInputProps, suggestions, getSuggestionItemProps }) => (
                <div className="autocomplete-wrapper">
                  ...
                  <div className="suggestions-list">
                    {suggestions.map((s) => (
                      <div {...getSuggestionItemProps(s)} key={s.placeId}>
                        {s.description}
                      </div>
                    ))}
                  </div>
          ...
          <div className="driver-dashboard-popup">
            ...
            <div className="popup-card">
                  ...
                  <div className="timeline-details">
                    <div className="timeline-block">
                      <div className="timeline-time">
                        {incomingRequest.driverToPickupEta || "..."} (
                        {incomingRequest.driverToPickupDistance || "..."}) away
                      </div>
                    ...
                    <div className="timeline-block">
                      <div className="timeline-time">
                        {incomingRequest.pickupToDropEta || "..."} (
                        {incomingRequest.pickupToDropDistance || "..."}) trip
                      </div>
                      <div className="timeline-address">
              ...
              <button
                ...
                disabled={timer === 0 || waitingForAssign}
              >
      ...
      {showTripOverlay && incomingRequest && pickupCoords && (
        <div className="driver-dashboard-trip-overlay">
          ...
          <p>
            <strong>Pickup:</strong> {incomingRequest.pickup}
          </p>
          <p>
            <strong>Drop:</strong> {incomingRequest.drop}
          </p>
          <p>
            <strong>ETA:</strong> {incomingRequest.driverToPickupEta}
          </p>
          <p>
            <strong>Urgency:</strong> {incomingRequest.urgency || "Normal"}
          </p>
          <p>
            <strong>Fare:</strong> ₹{incomingRequest.ambulanceCost}
          </p>
          ...
          {pickupCoords && driverLatLng && (
            ...
            >
              <button
                className="driver-dashboard-navigate-button"
