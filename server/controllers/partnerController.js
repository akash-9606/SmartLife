...
exports.addPartner = async (req, res) => {
  try {
    const { name, address, serviceAreas, commission, hospitalPlaceId, email } =
      req.body;
    ...
    if (
      ...
      return res
        ...
        .json({
          message:
            "All fields are required, including email and hospitalPlaceId.",
        });
    }
    ...
    if (existingHospitalPartner) {
      return res
        .status(400)
        .json({ message: "Partner already exists for this hospital." });
    }
    ...
    if (existingEmail) {
      return res
        .status(400)
        .json({ message: "Email is already associated with another partner." });
    }
    ...
    if (!geoData.results || geoData.results.length === 0) {
      return res
        .status(400)
        .json({ message: "Invalid address. Could not find coordinates." });
    }
    ...
    res.status(201).json({
      status: "success",
      message:
        "Partner added successfully. They can now register using this email.",
      data: newPartner,
    });
  } catch (error) {
    res.status(400).json({
...
exports.assignMultipleAmbulances = async (req, res) => {
  try {
    ...
    request.assignedAmbulances = formattedAmbulances;
    request.status = "multi_assigned";
    await request.save();

    res.status(200).json({
      status: "success",
      message: "Multiple ambulances assigned successfully.",
...
exports.confirmRequestToPartner = async (req, res) => {
  try {
    ...
    partner.pendingRequests = partner.pendingRequests.filter(
      (req) => req.userId && req.userName && req.pickup && req.drop
    );
...
exports.rejectAndTransferRequest = async (req, res) => {
  try {
    ...
    if (!request)
      return res.status(404).json({ message: "Request not found in partner." });
    ...
    const nearbyPartners = await Partner.aggregate([
      ...
      },
      {
        $match: {
          _id: {
            $nin: updatedReassignedPartners.map(
    ...
    if (!nearbyPartners.length) {
      return res
        .status(404)
        .json({ message: "No other nearby hospitals available." });
    }
...
exports.rejectRequest = async (req, res) => {
  try {
    ...
    if (!mainRequest)
      return res.status(404).json({ message: "Main request not found." });

    mainRequest.status = "rejected";
    mainRequest.rejectionType = rejectionType || "manual";
    const reassignedSet = new Set([
      ...(mainRequest.reassignedPartners || []).map((id) => id.toString()),
      partnerId.toString(),
    ]);
    mainRequest.reassignedPartners = Array.from(reassignedSet).map(
      (id) => new mongoose.Types.ObjectId(id)
    );
    await mainRequest.save();

    // Get nearby hospitals (you may already have this logic)
    const uniqueReassignedIds = [
      ...new Set(
        [...(mainRequest.reassignedPartners || []), partnerId].map((id) =>
          id.toString()
        )
      ),
...
exports.confirmHospitalSelection = async (req, res) => {
  try {
    ...
    console.log("REQ BODY in confirmHospitalSelection: ", req.body);
    ...
    const prevHospital = await Partner.findById(lastPartnerId);
    if (!prevHospital) {
    ...
    if (selectedPartnerId === lastPartnerId) {
      return res
        .status(400)
        .json({
          message: "Selected hospital is same as the previously rejected one.",
        });
    }
    ...
    const originalRequest = originalPartner.pendingRequests.find(
      (r) => r.bookingId === bookingId
    );
    if (!originalRequest) {
  ...
  try {
    ...
    res
      ...
      .json({
        ...
        request: partner.pendingRequests || [],
      });
  } catch (error) {
    res
      .status(500)
...
exports.markPartnerRequestAsAssigned = async (bookingId, driverId) => {
  try {
    ...
    await AmbulanceRequest.findOneAndUpdate(
      { bookingId },
      { status: "assigned_by_individual_driver", assignedDriver: driverId }
    );
  } catch (err) {
...
//       !urgency ||
//       !ambulanceType ||
//       numberOfAmbulancesRequested === undefined ||
//       numberOfAmbulancesRequested === null
//     ) {
//       return res.status(400).json({ message: "Missing required fields" });
//     }
...
//       prevHospitalName: prevHospital?.name || "Unknown Hospital",
//       hospitalName: selectedPartner.name,
//       numberOfAmbulancesRequested: originalRequest.numberOfAmbulancesRequested,
//     };
